...
public class SendServiceImpl implements SendService {
    ...
    public String sendWeChatMsg() {
        //验证是否成功请求
        //直接套用原作者写的方法getAccessToken()
        String accessToken = getAccessToken();
        if (!StringUtils.hasText(accessToken)) {
            logger.error("token获取失败，请检查：公众号的appId、appSecret");
            return "token获取失败，请检查：公众号的appId、appSecret";
        }
        ...
        for (String opedId : configConstant.getOpenidList()) {
            ...
            try {
                ...
                logger.info("wet:{}", wet);
                //未来三天天气
                //未使用该部分，不做任何处理 getTheNextThreeDaysWeather()
            ...
            //名言警句，判断有没开启每日一句功能，application.yml可以配置~
            //未使用该部分，不做任何处理 getOneNormalProverb()
